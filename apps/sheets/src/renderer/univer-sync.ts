...
/** Per-request cell budget for sidecar range reads. */
...
/// request spanning deleted file rows — or simply a large buffered viewport —
/// can exceed the sidecar's per-read cell budget, so reads are split by rows.
export async function readSheetRangeMapped(
  ...
  if (ops.length === 0) {
    ...
    for (let startRow = screenRange.startRow; startRow <= screenRange.endRow; startRow += batchRows) {
      ...
      raw = batch
      // Later rows cannot have data before indexing reaches them; the normal
      // retry path will request the remainder after indexing advances.
