...
export async function cleanupSessionResources(
  ...
  const snapshotRoot = join(options.tempRoot, 'nexoffice-sheets-sessions')
...
export async function cleanupImportTempDirectory(
  ...
  const importRoot = join(tempRoot, 'nexoffice-imports')
...
export async function cleanupExpiredPastedFiles(
  ...
  const dir = join(tempRoot, 'nexoffice-pasted')
