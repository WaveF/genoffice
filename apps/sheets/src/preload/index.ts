...
const desktopApi: DesktopApi = {
  onMcpRequest(handler) {
    const listener = (_event: Electron.IpcRendererEvent, request: { requestId: string; action: 'sheets.get_workbook_context' | 'sheets.read_range' | 'sheets.find' | 'sheets.apply_operations'; input: Record<string, unknown> }) => handler(request)
