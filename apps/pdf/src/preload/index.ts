...
const api: PdfApi = {
  onMcpRequest: (handler) => {
    const listener = (_event: Electron.IpcRendererEvent, request: { requestId: string; action: 'pdf.get_document_context' | 'pdf.read_page_context' | 'pdf.search' | 'pdf.apply_operations'; input: Record<string, unknown> }) => handler(request)
