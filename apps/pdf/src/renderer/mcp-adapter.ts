export function handlePdfMcpRequest(
  ...
  state: { pageCount: number; sizes: Array<{ width: number; height: number }>; pageBlocks: Map<number, Array<{ text: string }>> },
): unknown {
  ...
  return { page, size: state.sizes[page], blocks: (state.pageBlocks.get(page) ?? []).slice(0, 500).map((block) => ({ text: block.text.slice(0, 8192) })) }
}
