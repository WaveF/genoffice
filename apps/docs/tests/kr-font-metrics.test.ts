...
const sans = readWoff2(join(FONTS, 'NexOfficeSansKR-Regular-subset.woff2'))
const serif = readWoff2(join(FONTS, 'NexOfficeSerifKR-Regular-subset.woff2'))
...
describe('NexOffice Sans KR (Malgun-normalized)', () => {
...
describe('NexOffice Serif KR (Batang-normalized)', () => {
...
describe('NexOffice Che Latin KR (fixed-pitch half-width)', () => {
  const che = readWoff2(join(FONTS, 'NexOfficeCheLatinKR.woff2'))
...
describe('NexOffice Gothic KR (real source metrics, unmodified)', () => {
  const gothicKr = readWoff2(join(FONTS, 'NexOfficeGothicKR-Regular-subset.woff2'))
  ...
  it('keeps every primary font identifier clear of Reserved Font Names', () => {
    ...
    expect(names).toContainEqual({ nameId: 1, value: 'NexOffice Gothic KR' })
    expect(names).toContainEqual({ nameId: 6, value: 'NexOfficeGothicKR-Regular' })
  })
})
