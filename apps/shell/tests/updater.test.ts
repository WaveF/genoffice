...
beforeEach(() => {
  ...
  delete process.env.NEXOFFICE_FAKE_UPDATE
...
describe('manual download fallback', () => {
  const macFiles = [
    { url: 'NexOffice-0.2.0-arm64.zip' },
    { url: 'NexOffice-0.2.0.zip' },
    { url: 'NexOffice-0.2.0-arm64.dmg' },
    { url: 'NexOffice-0.2.0.dmg' },
  ]
  ...
  it('opens the feed-derived installer matching channel base URL and arm64 arch', async () => {
    ...
    try {
      ...
      expect(openExternal).toHaveBeenCalledWith(
        'https://cdn.example.com/mac/NexOffice-0.2.0-arm64.dmg',
      )
    } finally {
      restoreArch()
    }
  ...
  it('picks the arch-less dmg for x64 installs', async () => {
    ...
    try {
      const actions = await failTwiceIntoManual(macFiles)
      actions.onOpenDownload()
      expect(openExternal).toHaveBeenCalledWith('https://cdn.example.com/mac/NexOffice-0.2.0.dmg')
    } finally {
      restoreArch()
    }
  })
  ...
  it('rebuilds absolute metadata URLs against the trusted feed base', async () => {
    ...
    try {
      const actions = await failTwiceIntoManual([
        { url: 'https://attacker.example/NexOffice-0.2.0-arm64.zip' },
        { url: 'https://attacker.example/NexOffice-0.2.0-arm64.dmg' },
        { url: 'https://attacker.example/NexOffice-0.2.0.dmg' },
      ])
      actions.onOpenDownload()
      expect(openExternal).toHaveBeenCalledWith(
        'https://cdn.example.com/mac/NexOffice-0.2.0-arm64.dmg',
  ...
  it('rejects a non-HTTPS baked feed URL', async () => {
    ...
    const actions = await failTwiceIntoManual(macFiles)
    actions.onOpenDownload()
    expect(openExternal).toHaveBeenCalledWith('https://github.com/WaveF/genoffice/releases/latest')
  })
...
describe('initAutoUpdater (fake update preview)', () => {
  it('runs a simulated download to completion in unpacked runs', async () => {
    appState.isPackaged = false
    process.env.NEXOFFICE_FAKE_UPDATE = '9.9.9'
    const { initAutoUpdater } = await loadUpdater()
    initAutoUpdater(() => null)
