...
| SHT-04 | 为 Sheets 写端到端测试。 | P1 | SHT-02..03 | 进行中 | Codex | `apps/sheets/tests/` | 真实 xlsx 保存、流式保存、公式重算基线已通过；导入 workbook 的 lazy MCP renderer 路由已覆盖“载入后读取/未载入拒绝”。待增加 Electron 跨进程直接 MCP 路由 E2E。 | |
