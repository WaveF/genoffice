...
| ID     | 任务                                                                                   | 优先级 | 依赖               | 状态   | 负责人 | 代码落点                                   | 验收标准                                                | PR                                             |
| ------ | -------------------------------------------------------------------------------------- | ------ | ------------------ | ------ | ------ | ------------------------------------------ | ------------------------------------------------------- | ---------------------------------------------- |
| SUN-01 | 隐藏并移除 Slides 的 AiPanel、Ask AI、AI 菜单/Ribbon 入口。                            | P1     | SLD-08             | 进行中 | Codex  | `apps/slides/src/renderer/ai/`、App/Ribbon | Slides UI/runtime 已移除并通过 MCP 回归；待手工编辑回归。 | `712ebc1`..`966d8f9` |
| SUN-02 | 隐藏并移除 Docs/Markdown 的 AI 面板与快捷入口。                                        | P1     | DOC-03, MD-03      | 进行中 | Codex | 对应 renderer/App/Ribbon                   | Docs renderer AI runtime、面板、菜单、右键与 Review 入口已移除并通过 MCP 回归；Markdown 与 Docs Ribbon 隐藏残留待清理。 | `bdcf1c2`..`18b412c` |
| SUN-03 | 隐藏并移除 Sheets/PDF 的 AI 面板。                                                     | P2     | SHT-04, PDF-04     | 进行中 | Codex | 对应 renderer                              | Sheets/PDF 均不得保留可达的 AI UI、AgentLoop/transport 调用链或 `ai:*` IPC；Sheets 需删除 App 中的 AgentLoop、工具装配与聊天存储，PDF 需删除 `_aiApi` 与主进程 AI 链路。完成该条件即关闭任务。 | `d10a948`..`def0e9c` |
| SUN-04 | 删除内部 `ai:stream`、`ai:chat`、provider settings 和相关 preload IPC。                | P1     | SUN-01..03, DEC-04 | 进行中 | Codex | docs/shell/main、各 preload/shared IPC     | Docs/Shell 已停止注册 `ai:*`；待清理 preload、实现、依赖。 | `70fbd2d` |
