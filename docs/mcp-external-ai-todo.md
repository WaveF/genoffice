...
| SLD-06 | 实现 `slides.add_slide`、`slides.delete_slide`。                               | P1     | SLD-05, SEC-03         | 完成 | Codex  | Slides adapter + shell gateway                            | 显式 slide ID/索引、revision、队列已验证；删除操作走 token 认证后的 destructive-risk schema、审计与目标范围约束，不显示原生确认。 |     |
...
| MVP-03 | 验证 Slides 的 documentId、revision 与并发写入契约。                                      | P0     | CAP-02..03, CAP-07, SLD-01..05 | 已完成 | Codex  | 人工/MCP 编辑均单调推进 revision；相同 revision 的并发写入仅一方成功，另一方返回结构化 `conflict`；关闭后 documentId 不可解析。 | `mcp-adapter.test.ts` 覆盖人工事务 journal 后的 stale `conflict`、MCP mutation/undo/redo 的单调 revision；`tab-manager.test.ts` 覆盖 opaque documentId 与关闭后不可解析；真实双 stdio 客户端 E2E 验证相同 revision 竞争时恰一方写入成功、另一方收到 `{ code: "conflict" }`，并继续验证 read/undo/save。                                                         |
| MVP-04 | 验证写入安全边界：bridge token、无逐次应用内授权、输入限制、审计和 destructive 操作约束。 | P0     | INF-03..05, SEC-01..06         | 已完成 | Codex  | 已认证请求不显示应用内确认窗；未认证/越界输入被拒绝；审计记录完整且不含文档内容；删除类操作满足专用约束。                       | `mcp-bridge.test.ts` 验证私有 discovery、错误 token 拒绝且不进入 gateway；`mcp-permissions.test.ts` 验证 token 后 write/destructive/file 均无 UI gate；`mcp-input-guard.test.ts` 覆盖深度、数组、字段、字符串和总 payload 上限；`mcp-audit.test.ts` 与 gateway 审计测试验证仅记录元数据、轮转和 0600 文件权限；PDF/Slides 删除均须显式目标及 destructive risk。 |
| MVP-05 | 以独立 stdio MCP 客户端完成 Slides 端到端验收。                                           | P0     | MVP-01..04, SLD-01..08         | 已完成 | Codex  | 覆盖列出已打开文档、读取、dry-run、写入、conflict、undo、redo、save；不依赖 active Tab 作为写入目标。                           | `e2e/mcp-shell-slides.spec.ts` 通过独立 stdio adapter 启动真实 Shell bridge：创建两个 Slides deck，验证 list/read/dry-run；第二个 deck 保持 active 时，对第一个 explicit documentId 执行双客户端竞争写入（恰一方成功、一方 conflict）、undo、redo、save，并确认 active deck revision/slideCount 未改变。                                                        |
| MVP-06 | 复核内置 AI 下线后的产品边界与 MCP 能力缺口。                                             | P1     | SUN-01..06, MED-01             | 已完成 | Codex  | 无可达的内置 AI UI/runtime/provider；确认外部生成图片插入仍为不阻塞的 `MED-01`，不把缺口误报为 MVP 已支持能力。                 | `internal-ai-sunset-execution.md` 的 SUN7-01..09 证实 UI/runtime/provider/AI IPC/依赖已删除；MCP tools/list 回归断言无图像生成或通用媒体导入 tool，README 明确外部 Agent 自带生成能力；`mcp-capability-gaps.md` 保持 `MED-01` 未开始并定义受控 media handle 方向。Genspark 账号/云转换为已记录的后续 Free + MCP 账户清理范围，不属于内置模型或 MCP 功能。       |
| MVP-07 | 复跑并归档质量门禁。                                                                      | P0     | MVP-02..06, QLT-01..06         | 已完成 | Codex  | 本地 typecheck/lint/format/test 与 CI 的 unit、E2E、macOS/Windows/Linux 成品包 MCP 冒烟均通过；记录具体 CI Run。                | 本地 format/lint/typecheck/fixtures/test 与 37 项 E2E 通过；为移除 DNS 偶发性，mocked-fetch 改用公开 IP，不改变 SSRF guard。[CI Run 33168595889](https://github.com/WaveF/genoffice/actions/runs/33168595889)（`70a91a8`）的 test、E2E 和三平台 package MCP smoke 均成功。                                                                                      |
| MVP-08 | 生成 MVP 验收报告，更新完成清单和对外连接说明。                                           | P0     | MVP-01..07                     | 已完成 | Codex  | 报告逐项引用证据、已知非阻塞缺口和复现步骤；本表全为已完成；README/连接文档不承诺未提供的图片导入或任意文件访问能力。           | `slides-mcp-mvp-acceptance.md` 逐项归档 MVP-01..07、复现步骤、MED-01 与 CI Run 33168595889；README 与 usage 说明已移除内置 AI/BYOK 口径，并明确不支持任意文件路径或通用媒体导入。                                                                                                                                                                               |
