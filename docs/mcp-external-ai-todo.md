...
| SHT-03 | 实现 Sheets `propose_operations` 的 MCP 等价工具（改名为 `sheets.apply_operations`）。 | P1 | SHT-02, CAP-07, SEC-02 | 进行中 | Codex | Sheets adapter | dry-run、CAS、权限、队列、内存与导入 workbook 路径已接入；导入 workbook 的真实 mutation 已单调维护 MCP revision，`sheets.undo` 已兼容 Univer 与 adapter 历史；MCP 公式写入/追踪/撤销与 XLSX 重算基线已覆盖。待补导入 workbook 的 MCP 写后重算数值回归。 | |
| SHT-04 | 为 Sheets 写端到端测试。 | P1 | SHT-02..03 | 进行中 | Codex | `apps/sheets/tests/` | 真实 xlsx 保存、流式保存、公式重算基线已通过；lazy MCP renderer 与 Shell→adapter 读写/权限/revision 路由已覆盖。待增加 Electron 原生 IPC 进程级回归。 | |
