...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'nexoffice-save-dir-'))
})
...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'NexOffice')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/NexOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'NexOffice'))
    expect(existsSync(join(documents, 'NexOffice'))).toBe(true)
  })
})
