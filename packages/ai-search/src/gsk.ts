...
import { nexofficeApiKey } from './nexoffice-auth'
...
function electronCompatArgs(): string[] {
  ...
  if (compatPath === undefined) {
    try {
      const dir = join(homedir(), '.nexoffice', 'bin')
 ...
 * Priority: GSK_API_KEY env → NexOffice's own key (bills to us via its
...
async function toolCliPost(
  ...
  try {
    const resp = await fetch(`${GSK_TOOL_CLI_BASE}${path}`, {
      method: 'POST',
      // X-Agent-Type splits NexOffice usage out of the proxy's "Claw" billing bucket
      headers: {
        'X-Api-Key': key,
        'Content-Type': 'application/json',
        'X-Agent-Type': 'nexoffice',
      },
