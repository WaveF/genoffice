...
test.describe('pdf: MCP renderer IPC', () => {
  test('routes document context, bounded annotation write, and annotation read over Electron IPC', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'genoffice-pdf-mcp-'))
    const pdfPath = join(directory, 'document.pdf')
    await writeFile(pdfPath, minimalPdf())
    const launched = await launchShell({ onboardingSeen: true, videoDir: 'pdf-mcp-renderer', openFile: pdfPath })
    try {
      ...
      const webContentsId = await launched.app.evaluate(({ webContents }) =>
        webContents.getAllWebContents().find((contents) => contents.getURL().includes('pdf/out'))?.id,
      )
      if (!webContentsId) throw new Error('PDF renderer was not found')

      const context = await requestPdfRenderer(launched.app, webContentsId, 'pdf.get_document_context', {})
      ...
      expect(write).toMatchObject({ ok: true, result: { dryRun: false, revision: 1, changes: { notes: 1 } } })
      await pdf.waitForTimeout(100)

      const annotations = await requestPdfRenderer(launched.app, webContentsId, 'pdf.read_annotations', { page: 0 })
