...
async function requestSheetsRenderer(
  ...
  return app.evaluate(({ ipcMain, webContents }, payload) => new Promise((resolve, reject) => {
    ...
    target.send('mcp:renderer-request', { requestId: payload.requestId, action: payload.action, input: payload.input })
...
test.describe('sheets: imported workbook MCP formulas', () => {
  test('writes a formula and reads its recalculated live value over Electron IPC', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'genoffice-sheets-mcp-formula-'))
    const workbookPath = join(directory, 'formula.xlsx')
    await writeFile(workbookPath, await buildEditFixture())
    const launched = await launchShell({ onboardingSeen: true, videoDir: 'sheets-mcp-formula', openFile: workbookPath })
    try {
      const sheets = await waitForPageWithUrl(launched.app, 'sheets/out')
      await sheets.waitForFunction(() => document.body.textContent?.includes('Data'), null, { timeout: 30_000 })
      ...
      await expect.poll(async () => {
        const result = await requestSheetsRenderer(launched.app, webContentsId, 'sheets.get_workbook_context', {})
        return (result.result as { sheets?: Array<{ name?: unknown }> } | undefined)?.sheets?.[0]?.name
      }, { timeout: 30_000 }).toBe('Data')
      const context = await requestSheetsRenderer(launched.app, webContentsId, 'sheets.get_workbook_context', {})
      expect(context).toMatchObject({ ok: true, result: { revision: 0, sheets: [{ name: 'Data' }] } })
      ...
      await expect.poll(async () => {
        ...
        return ((read.result as { cells?: Array<{ value?: unknown }> } | undefined)?.cells?.[0]?.value)
