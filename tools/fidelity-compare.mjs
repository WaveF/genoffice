 ...
 * Our side: playwright drives the packaged NexOffice Slides (Electron) with zoom locked at 100%,
 ...
 * Prereq: npm run build -w @nexoffice/slides; brew: libreoffice + poppler (pdftoppm).
 */
...
/**
 * Opens NexOffice Slides and, page by page, composites the main canvas's Konva layers into a PNG
...
async function shootOurs(pptx, dir, thumbIndexes) {
  ...
  const userData =
    process.env.NEXOFFICE_USER_DATA ?? path.join(os.tmpdir(), 'nexoffice-fidelity-userdata')
  fs.mkdirSync(userData, { recursive: true })
  const app = await electron.launch({
    executablePath: ELECTRON_BIN,
    args: [APP_DIR, pptx],
    env: { ...process.env, NEXOFFICE_USER_DATA: userData },
    timeout: 30_000,
  })
  try {
    const page =
    ...
    await page.evaluate(() => {
      window.__nexofficeHidePhPrompts = true
      window.dispatchEvent(new Event('nexoffice:hide-ph-prompts'))
    })
    await sleep(300)
    const shots = []
    for (const idx of thumbIndexes.filter((i) => i < total)) {
      ...
      for (let i = 0; i < 20; i++) {
        if (await page.evaluate(() => window.__nexofficeDocFontsSynced !== false)) break
        await sleep(150)
      }
...
<table><tr><td>reference<br><img src="${rel(r.ref)}"></td><td>NexOffice Slides<br><img src="${rel(r.ours)}"></td><td>diff<br><img src="${rel(r.diff)}"></td></tr></table>`,
  )
  .join('')}
`
